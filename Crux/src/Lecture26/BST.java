...
public class BST {
	...
	public void pathNode(int data) {
		System.out.println(this.pathNode(this.root, data,""));
	}

	private String pathNode(Node node, int data, String res) {
		if (node == null) {
			res="";
			return res;
		}
		if (data < node.data) {
			res += pathNode(node.left, data,res)+node.data+",";
		} else if (data > node.data) {
			res += pathNode(node.right, data,res)+node.data+",";
		} else
			res +=  node.data+",";
		return res;
	}
